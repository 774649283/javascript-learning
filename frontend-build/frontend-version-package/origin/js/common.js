...
function valifyServer(server) {
  var req = /^(\w+\.){2,}(\w+)$/;
  return req.test(server);
}
...
function trim(str) {
    ...
    // 上面这个哪个傻子写的方法！！直接把null转换成了'null'，引起多少bug，心里没点b数！！
    // 正则也写的像傻子！！
...
var AjaxRequest = (function(){
    ...
    function ajax(options){
		...
		if ($.cookie('access_token') == null || $.cookie('access_token') == "") {
			...
			location.href = "/infinity3/login?url=" + tmpurl;
			return;
		}
...
var DataModel = (function(){
    ...
    var postUrl = {
        'checkReboot': 'nodeinfo/checkReboot',
        'checkService': 'nodeinfo/checkService',
        'getUser' : 'nodeinfo/getUser',
        // sysdb
        'addSysdbNode' : 'nodeinfo/addSysdbNode',
				// user model
        ...
        // sthome
                ...
                'listStorageInfo2' :'sthome/listStorageInfo2',
                'listStorageSpace': 'sthome/listStorageSpace',
                'listNodesInfo': 'sthome/listNodesInfo',
                'listDisksInfo': 'sthome/listDisksInfo',
                'getExtBandwidth': 'sthome/getExtBandwidth',
                'getDataStatus': 'sthome/getDataStatus',
                'getMigrationStatus': 'sthome/getMigrationStatus',
                'getInitTime': 'sthome/getInitTime',
                'getRealAlarm': 'sthome/getRealAlarm',
                //group modal
                ...
                // diskgroup
                'dgRepeatCheck': 'storagepoolmgm/dgRepeatCheck',
                'dgRepeatDiskCheck': 'storagepoolmgm/dgRepeatDiskCheck',
                'addDiskgroup': 'storagepoolmgm/addDiskgroup',
                'editDiskGroup': 'storagepoolmgm/editDiskgroup',
                'createDiskgroup': 'storagepoolmgm/createDiskgroup',
                'deleteDiskgroup': 'storagepoolmgm/deleteDiskgroup',
                'listDiskgroup': 'storagepoolmgm/listDiskgroup',
                'removeDiskgroup': 'storagepoolmgm/removeDiskgroup',
                ...
                'selectAuditLogs': 'auditlogs/selectAuditLogs',  // 筛选审计日志
                'selectOptLogs': 'optlogs/selectOptLogs',  // 筛选操作日志
                'downloadLog': 'optlogs/downloadLog',  // 下载操作日志
                'downloadAuditLog': 'auditlogs/downloadLog',  // 下载操作日志
                'selectHistoryAlarmLogs': 'historyalarm/selectLogs',  // 筛选历史告警
                'downloadHistoryAlarmLogs': 'historyalarm/downloadLog',  // 筛选历史告警
                'selectAlarmLogs': 'alarm/selectLogs',  // 筛选告警详情日志
                'downloadAlarmLogs': 'alarm/downloadLog',  // 下载告警日志

                //存储管理
                'poolStatusCheck': 'storagepoolmgm/poolStatusCheck',
                'addTier': 'storagepoolmgm/addTier',
                ...
                'listPoolApplication': 'storagepoolmgm/listStoragePoolByApplication',
                'listAllPoolsHome': 'storagepoolmgm/listAllPoolsUsedspace',
                'listAllStorage': 'storagepoolmgm/listAllStorage',
                'listAllStorages': 'storagepoolmgm/listAllStorages',
                'poolQuotaget':'storagepoolmgm/poolQuotaget',
                'poolQuotaset': 'storagepoolmgm/poolQuotaset',
                'poolSet': 'storagepoolmgm/poolSet',
                'checkFailureDomain': 'storagepoolmgm/checkFailureDomain',
                //文件系统
                ...
                'delObjNode': 'nodeinfo/delObjNode',
                'reduceCheckFailureDomain': 'clustermgm/reduceCheckFailureDomain',

                //授权管理
                'licenseList': 'licensemgm/licenseList',
                'licenseAdd': 'licensemgm/licenseAdd',
                'licenseParse': 'licensemgm/licenseParse',
                'licenseRemove': 'licensemgm/licenseRemove',
                'licenseActive': 'licensemgm/licenseActive',
                'licenseDetail': 'licensemgm/licenseDetail',
                'licenseAddedNodeList': 'licensemgm/licenseAddedNodeList',
                'licenseAddedNodeRemove': 'licensemgm/licenseAddedNodeRemove',
                'licenseUnaddedNodesList': 'licensemgm/licenseUnaddedNodesList',
                'licenseActivedDetail': 'licensemgm/licenseActivedDetail',
                'licenseActiveAll':'licensemgm/licenseActiveAll',
                'licenseGetTrial': 'licensemgm/getLicenseTrial',
                'licenseGetTrialAll': 'licensemgm/getLicenseTrialAll',
                'licenseInit': 'licensemgm/initLicense',
                'getClusterActivedNodes':'licensemgm/getClusterActivedNodes',
                'checkIpsActivation': 'licensemgm/checkIpsActivation',

                // 运维与关机
                'maintenanceServList' : 'maintenance/listMaintenanceServ',
                'maintenanceStart' : 'maintenance/maintenaceStart',
                'maintenanceStop' : 'maintenance/maintenaceStop',
                'maintenanceStatus' : 'maintenance/maintenaceStatus',
                'maintenanceBalanceStatus' : 'maintenance/maintenaceBalanceStatus',
                'maintenanceBalanceSwitch' : 'maintenance/maintenaceBalanceSwitch',
                'maintenanceNodeShutdown' : 'maintenance/maintenaceNodeShutdown',
                'maintenaceNodeReboot' : 'maintenance/maintenaceNodeReboot',
                'maintenanceClusterServList' : 'maintenance/listMaintenanceClusterServ',
                'maintenanceClusterProcess' : 'maintenance/listMaintenanceClusterProcess',

                // init
                'setClusterInitStatus': 'clustermgm/initStatusSet',
                'clusterLvmRemove': 'clustermgm/clusterLvmRemove',
				};
    var funs = {};
...
//格式化字节单位 单位bytes - 1000
function formatCapacity1000(capacity) {
    var str = "";
    if (capacity == 0 || capacity == "") {
        str = "0";
    } else {
        if ((capacity / (1000*1000*1000*1000*1000)) >= 1) { // PB
            str = ((Math.floor((capacity /(1000*1000*1000*1000*1000))*100))/100) + " PB";//100保留两位
        } else if ((capacity / (1000*1000*1000*1000)) >= 1) { // T
            str = ((Math.floor((capacity /(1000*1000*1000*1000))*100))/100) + " TB";//100保留两位
        } else if (capacity / (1000*1000*1000) >= 1) { //G
            str = ((Math.floor((capacity /(1000*1000*1000))*100))/100) + " GB";
        } else if (capacity / (1000*1000) >= 1) { //M
            str = ((Math.floor((capacity /(1000*1000))*100))/100) + " MB";
        } else if (capacity / (1000) >= 1) {       //k
            str = ((Math.floor((capacity /(1000))*100))/100) + " KB";
        } else if (capacity > 0) {
            str = capacity + " Bytes";
        }
    }
...
// 测试保留字
// [‘ceph’、‘rbd’、‘crush’、‘osd’、‘pg’、‘radosgw’、‘rados’、‘CEPH’‘RBD’‘CRUSH’‘OSD’‘PG’、‘RADOS’、‘RADOSGW
var valifyKeyWord = function (string) { // 屏蔽关键字
  var keyReg = /(ceph|rbd|crush|osd|pg|radosgw|rados)/gi;
  if (keyReg.test(string)) {
    return keyReg;
  }else {
    return false;
  }
...
// 系统保留字替换
var replaceSystemKeyword = function (str) {
  ...
  match = str.match(/(infinity|volume|pool-conf|disk|chunk|objgw|obj)/ig);
  match && match.forEach(function (m) {
    str = str.replace(m, recordMap[m. toLowerCase()]);
  });
  return str;
};

// 秒->天
function secondsToDay(sec) {
  sec = (sec && sec >= 0) ? sec : 0;
  sec = sec / (60 * 60 * 24);
  sec = sec ? (sec.toFixed('2')) : 0;
  return sec;
}
...
//判断对象是否为空或undefined  空true，否则false
function empty(str) {
    //jquery 中
    // str = $.trim(str);
    return ($.isEmptyObject(str) || str == 'undefined' || $.trim(str)=='' || str == null);
}
