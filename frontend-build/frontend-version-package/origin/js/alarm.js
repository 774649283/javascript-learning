  ...
  var getLogsAndRender = function (params, isJump) {
    ...
    var dncallback = function (result) {
      ...
      if (result.code == 200) {
        ...
        var htm = '';
        var level = '';
        var data = result.result[0].records;
        ...
        for (var i = 0; i < data.length; i++) {
          var latetime = data[i]["latest_time"];
          var time = '-';
          if ( latetime == 0 ) {
            time = moment.unix( data[i]["time"] ).format('MM/DD/YY HH:mm:ss');
          } else{
            time = moment.unix( data[i]["latest_time"] ).format('MM/DD/YY HH:mm:ss');
          }

          htm +=  '<div class="alarm-mag" date-time="'+ time +'">' +
                '<div class="col-lg-2 basic-lg-2 col-format-row ellipsis">'+
                  '<span>' + time + '</span>'+
                '</div>'+
                '<div class="col-lg-2 basic-lg-2 col-format-row ellipsis">'+
                  '<span>' + data[i]["module_name"] + '</span>'+
                '</div>'+
                '<div class="col-lg-1 basic-lg-1 col-format-row ellipsis">'+
                  '<span>' + data[i]["hostname"] + '</span>'+
                '</div>'+
                '<div class="col-lg-2 basic-lg-2 col-format-row ellipsis">'+
                  '<span>' + data[i]["severity"] + '</span>'+
                '</div>'+
                '<div class="col-lg-5 basic-lg-5 col-format-row ellipsis left">'+
                  '<span title="' + data[i]["description"] + '">' + data[i]["description"] + '</span>'+
                '</div>'+
              '</div>';
        };
        $('#alarmlist').html(htm);
    ...
    // 根据参数情况选择接口调用
    DataModel["selectAlarmLogs"](parameter, dncallback, true);
  ...
  getLogsAndRender({
    ...
    .on('click','#downloadLogs',function(ev){
  		...
  		function callback(result){
  			...
  			if(result.code == 200){
  				htm = '<a href="'+link+'" download="alarmlog.csv"><button type="button" id="startDownload" class="btn btn-info btn-block" ' +
