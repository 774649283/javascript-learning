...
Utils.fnSmooth = (function () {
   ...
   * @param  {[Number]}   delayTime  [设置的延迟时间]
   * @param  {[Any]}      args       [函数传参]
   * @param  {[Bool]}     IsImediate [是否立即调用]
   */
  return function (fn, delayTime, args, isImediate) {
    args = Array.isArray(args) ? args : [args];
    var setTimer = function () {
      timer = setTimeout(function () {
        fn(args);
        clearTimeout(timer);
        if (fnObject.includes(fn)) {
          fnObject.splice(fnObject.indexOf(fn), 1);
        };
      }, delayTime);
...
Utils.fnDelay = (function (){
  ...
  return function(fn, delayTime, args, IsImediate){
      ...
      if(fnObject.includes(fn)) return;
